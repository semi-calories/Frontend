        ...
        <View style={styles.rightView}>
          ...
          <View style={styles.flexRow}>
            ...
            <View>
              ...
              <Text style={[styles.text, { fontSize: rFont(14) }]}>유저 사진 설정, 식단 기록 시 사용</Text>
            </View>
          </View>
          <View style={styles.flexRow}>
            <View style={styles.icon}>
              <MaterialIcons name="notifications-none" size={rHeight(30)} color={colors.black} />
            </View>
            <View>
              <Text style={[styles.text, { color: colors.black }]}>알림(선택)</Text>
              <Text style={[styles.text, { fontSize: rFont(14) }]}>식단 추천 푸시 알람 시 사용</Text>
            </View>
          </View>
          <Text style={[styles.noticeText, { marginTop: rHeight(15), color: 'black' }]}>* 필수적 접근 권한은 사용하지 않습니다.</Text>
          <Text style={[styles.noticeText, { marginTop: rHeight(30) }]}>* 선택적 접근 권한은 해당 기능을 사용할 때만 허용이 필요합니다.</Text>
          <Text style={[styles.noticeText]}>* 비허용시에도 해당 기능 외 서비스 이용이 가능합니다. 허용 상태는 휴대폰 설정 메뉴에서 언제든지 변경할 수 있습니다.</Text>
        </View>
      </View>
      <View style={{ alignSelf: 'center' }}>
        <PrimaryButton text="확인" onPress={onPressComfirm} btnStyle={{ width: rWidth(340) }} />
      </View>
    </RootView>
  )
}
