...
const HomeStatisticPeriod = ({ type, userInfo }) => {
  ...
  const getDietWeekStats = async () => {
    const rawMonthInfo = CalculateWeek(
  ...
  const formatDate = useMemo(
    () => moment(selectDate).format(type === '주간' ? 'YYYY-MM' : 'YYYY'),
    [selectDate, type],
  );
  ...
  const YearMonthFunc = () => {
    ...
    for (let year = 2010; year <= currentYear; year++) {
      Array.from({ length: 12 }, (v, i) =>
        yearMonthArr.push(`${year}년 ${i + 1}월`),
      );
    }
  ...
  return (
    <View>
      ...
      <View style={styles.dayView}>
        <Pressable
          onPress={() =>
            setSelectDate(CalculateStatisticDate(type, 'sub', selectDate))
          }
        >
        ...
        <Pressable
          onPress={() =>
            setSelectDate(CalculateStatisticDate(type, 'add', selectDate))
          }
        >
      ...
      >
        <Pressable
          onPress={() => handlePicker(pickerDate)}
        ...
        {type === '주간' ? (
          <WheelPickerExpo
            height={rHeight(260)}
            width={dWidth}
            initialSelectedIndex={yearMonthArray.indexOf(
              moment(selectDate).format('YYYY년 M월'),
            )}
          ...
          <WheelPickerExpo
            height={rHeight(260)}
            width={dWidth}
            initialSelectedIndex={yearArray.indexOf(
