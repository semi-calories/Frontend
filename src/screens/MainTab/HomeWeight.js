...
const HomeWeight = ({ userInfo }) => {
  ...
  const lineDataMaxValue = useMemo(
    () => Math.ceil(Math.max(...lineData.map((v) => v.value)) / 10) * 10 + 20,
    [lineData],
  );
  ...
  const handleRangeWeight = async () => {
    ...
    try {
      const { weightList: rawWeightList } =
        await getMonthRangeWeight(weightInfo);
      const [i, ...weightList] = getStructedRangeWeight(rawWeightList);
      const [index, ...predictWeightList] =
        getStructedPredictWeight(rawWeightList);
      ...
      <View style={styles.chartView}>
        {isLoaded ? (
          <LineChart
            ...
            adjustToWidth={!(lineData.length > 6)}
            scrollToEnd
            hideYAxisText
            xAxisType="dashed"
          ...
          <View style={[styles.rangeView, { marginTop: rHeight(20) }]}>
            {FILTERPERIOD.map((per, idx) => (
              <BasicChip
                ...
                chipStyle={{
                  backgroundColor:
                    per === filterPeriod ? colors.btnBackground : colors.white,
                  borderColor:
                    per === filterPeriod ? colors.white : colors.btnBackground,
                }}
                text={per}
              />
            ))}
        ...
        <View style={{ flex: 1 }}>
          <View style={styles.box}>
            ...
            <DateTimePickerSelect
              mode="date"
              value={selectDate}
              onChange={(event, selectedDate) => setSelectDate(selectedDate)}
            />
          </View>
