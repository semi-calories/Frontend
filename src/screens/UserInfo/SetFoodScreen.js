      ...
      navigation.setOptions({
        header: () => (
          <BackHeader
            ...
            rightType={HeaderType.skip}
            rightPress={() => navigation.navigate('MainTab')}
          />
        ),
      });
    ...
    <RootView viewStyle={styles.container}>
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.chipView}>
          {preferFood &&
            [...preferFood].map((food, idx) => (
              <CloseChip
                key={food + idx}
                onClose={() =>
                  setPreferFood([...preferFood.filter((fd) => fd !== food)])
                }
                text={food.foodName}
              />
            ))}
        ...
        <View style={styles.chipView}>
          {dislikeFood &&
            [...dislikeFood].map((food, idx) => (
              <CloseChip
                key={food + idx}
                onClose={() =>
                  setDislikeFood([...dislikeFood.filter((fd) => fd !== food)])
                }
                text={food.foodName}
              />
            ))}
