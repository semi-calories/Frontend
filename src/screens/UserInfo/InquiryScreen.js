...
const InquiryScreen = ({ navigation }) => {
    ...
    useLayoutEffect(() => {
        navigation.setOptions({
            header: () => <BackHeader back backPress={() => navigation.goBack()} title="문의하기" />
        });
    }, [navigation]);
            ...
            case 'email':
                return (
                    <View style={styles.inputView}>
                        ...
                        />
                        {email && !emailRegex.test(email) && <Text style={styles.validateText}>{ValidFormat('이메일')}</Text>}
                    </View>
                );
            case 'type':
                return (
    ...
    return (
        <RootView viewStyle={styles.container}>
            ...
            />
            <MoveButton text='제출하기' onPress={onPressSubmit} inActive={email && value && title && content ? false : true} />
        </RootView>
    );
}
